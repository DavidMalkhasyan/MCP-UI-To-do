"use strict";Object.defineProperty(exports,Symbol.toStringTag,{value:"Module"});const f="ui/resourceUri",I="mcpui.dev/ui-",w=`var __defProp = Object.defineProperty;
...
`;function m(e){const n=e?JSON.stringify(e):"{}";return`
<script>
(function() {
  'use strict';
...
<\/script>
`.trim()}const y=`var __defProp = Object.defineProperty;
var __defNormalProp = (obj, key, value) => key in obj ? __defProp(obj, key, { enumerable: true, configurable: true, writable: true, value }) : obj[key] = value;
var __publicField = (obj, key, value) => {
  __defNormalProp(obj, typeof key !== "symbol" ? key + "" : key, value);
  return value;
...
`;function h(e){const n=e?{timeout:e.timeout}:{},s=JSON.stringify(n);return`
<script>
(function() {
  'use strict';
  ...
  if (typeof window !== 'undefined') {
    ...
    window.McpAppsAdapter = {
      init: initAdapter,
      initWithConfig: () => initAdapter(${s}),
      uninstall: uninstallAdapter,
    };
  }
})();
<\/script>
`.trim()}function g(e){const n={...e.resourceProps??{}};if(e.uiMetadata||e.metadata){const s=Object.fromEntries(Object.entries(e.uiMetadata??{}).map(([a,i])=>[`${I}${a}`,i]));n._meta={...s,...e.metadata??{},...n._meta??{}}}return n}function u(e){if(typeof Buffer<"u")return Buffer.from(e,"utf-8").toString("base64");if(typeof TextEncoder<"u"&&typeof btoa<"u"){const s=new TextEncoder().encode(e);let a="";const i=8192;for(let t=0;t<s.length;t+=i)a+=String.fromCharCode(...s.slice(t,t+i));return btoa(a)}else{console.warn("MCP-UI SDK: Buffer API and TextEncoder/btoa not available. Base64 encoding might not be UTF-8 safe.");try{return btoa(e)}catch{throw new Error("MCP-UI SDK: Suitable UTF-8 to Base64 encoding method not found, and fallback btoa failed.")}}}function l(e){if(e){if(e.appsSdk?.enabled)return e.appsSdk.mimeType??"text/html+skybridge";if(e.mcpApps?.enabled)return"text/html+mcp"}}function c(e,n){if(!n)return e;const s=[];if(n.appsSdk?.enabled){const t=m(n.appsSdk.config);s.push(t)}if(n.mcpApps?.enabled){const t=h(n.mcpApps.config);s.push(t)}if(s.length===0)return e;const a=s.join(`
`);let i;return e.includes("<head>")?i=e.replace("<head>",`<head>
${a}`):e.includes("<html>")?i=e.replace("<html>",`<html>
<head>
${a}
</head>`):i=`${a}
${e}`,i}async function M(e,n){let s;try{if(s=new URL(e),s.protocol!=="http:"&&s.protocol!=="https:")throw new Error(`MCP-UI SDK: URL must use http or https protocol. Got: ${s.protocol}`)}catch(r){throw new Error(`MCP-UI SDK: Invalid URL provided for externalUrl fetch: ${r instanceof Error?r.message:String(r)}`)}let a;try{a=await fetch(e,{headers:{"User-Agent":"MCP-UI-Server/1.0",Accept:"text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8"}})}catch(r){throw new Error(`MCP-UI SDK: Failed to fetch external URL: ${r instanceof Error?r.message:String(r)}`)}const i=a.headers.get("content-type")||"";!i.includes("text/html")&&!i.includes("application/xhtml")&&console.warn(`MCP-UI SDK: External URL content-type is not HTML: ${i}. Proceeding anyway.`);let t;try{t=await a.text()}catch(r){t=`<!DOCTYPE html>
<html>
<head>
  <title>Error ${a.status}</title>
</head>
<body>
  <h1>Error ${a.status}: ${a.statusText}</h1>
  <p>Failed to fetch content from ${e}</p>
  <p>Error: ${r instanceof Error?r.message:String(r)}</p>
</body>
</html>`}if(!a.ok)console.warn(`MCP-UI SDK: External URL returned non-OK status: ${a.status} ${a.statusText}. Proceeding with response body.`),(!t||t.trim()==="")&&(t=`<!DOCTYPE html>
<html>
<head>
  <title>Error ${a.status}</title>
</head>
...
</html>`);else if(!t||t.trim()==="")throw new Error("MCP-UI SDK: External URL returned empty content");t=t.replace(/<base[^>]*>/gi,""),t=t.replace(/<meta[^>]*http-equiv\s*=\s*["']?content-security-policy["']?[^>]*>/gi,"<!-- CSP meta tag removed by MCP-UI -->");const o=s.pathname.substring(0,s.pathname.lastIndexOf("/")+1)||"/",d=`<base href="${`${s.origin}${o}`}">`;return t.includes("<head>")?t=t.replace("<head>",`<head>
${d}`):t.includes("<html>")?t=t.replace("<html>",`<html>
<head>${d}</head>`):t=`<head>${d}</head>
${t}`,n&&(t=c(t,n)),t}function A(e){let n,s;if(e.content.type==="rawHtml"){if(!e.uri.startsWith("ui://"))throw new Error("MCP-UI SDK: URI must start with 'ui://' when content.type is 'rawHtml'.");if(n=e.content.htmlString,typeof n!="string")throw new Error("MCP-UI SDK: content.htmlString must be provided as a string when content.type is 'rawHtml'.");e.adapters?(n=c(n,e.adapters),s=l(e.adapters)??"text/html"):s="text/html"}else if(e.content.type==="externalUrl"){if(!e.uri.startsWith("ui://"))throw new Error("MCP-UI SDK: URI must start with 'ui://' when content.type is 'externalUrl'.");const i=e.content.iframeUrl;if(typeof i!="string")throw new Error("MCP-UI SDK: content.iframeUrl must be provided as a string when content.type is 'externalUrl'.");if(e.adapters&&(e.adapters.appsSdk?.enabled||e.adapters.mcpApps?.enabled))return(async()=>{const t=await M(i,e.adapters);let o;e.adapters?o=l(e.adapters)??"text/html":o="text/html";let p;switch(e.encoding){case"text":p={uri:e.uri,mimeType:o,text:t,...g(e)};break;case"blob":p={uri:e.uri,mimeType:o,blob:u(t),...g(e)};break;default:{const d=e.encoding;throw new Error(`MCP-UI SDK: Invalid encoding type: ${d}`)}}return{type:"resource",resource:p,...e.embeddedResourceProps??{}}})();n=i,s="text/uri-list"}else if(e.content.type==="remoteDom"){if(!e.uri.startsWith("ui://"))throw new Error("MCP-UI SDK: URI must start with 'ui://' when content.type is 'remoteDom'.");if(n=e.content.script,typeof n!="string")throw new Error("MCP-UI SDK: content.script must be provided as a string when content.type is 'remoteDom'.");s=`application/vnd.mcp-ui.remote-dom+javascript; framework=${e.content.framework}`}else{const i=e.content;throw new Error(`MCP-UI SDK: Invalid content.type specified: ${i}`)}let a;switch(e.encoding){case"text":a={uri:e.uri,mimeType:s,text:n,...g(e)};break;case"blob":a={uri:e.uri,mimeType:s,blob:u(n),...g(e)};break;default:{const i=e.encoding;throw new Error(`MCP-UI SDK: Invalid encoding type: ${i}`)}}return{type:"resource",resource:a,...e.embeddedResourceProps??{}}}function R(e){window.parent&&window.parent.postMessage(e,"*")}const P={UI_MESSAGE_RECEIVED:"ui-message-received",UI_MESSAGE_RESPONSE:"ui-message-response",UI_SIZE_CHANGE:"ui-size-change",UI_LIFECYCLE_IFRAME_READY:"ui-lifecycle-iframe-ready",UI_LIFECYCLE_IFRAME_RENDER_DATA:"ui-lifecycle-iframe-render-data",UI_RAWHTML_CONTENT:"ui-html-content"},S={WAIT_FOR_RENDER_DATA:"waitForRenderData"};function b(e,n){return{type:"tool",payload:{toolName:e,params:n}}}function C(e){return{type:"prompt",payload:{prompt:e}}}function U(e){return{type:"link",payload:{url:e}}}function D(e,n){return{type:"intent",payload:{intent:e,params:n}}}function E(e){return{type:"notify",payload:{message:e}}}exports.InternalMessageType=P;exports.RESOURCE_URI_META_KEY=f;exports.ReservedUrlParams=S;exports.createUIResource=A;exports.getAdapterMimeType=l;exports.getAppsSdkAdapterScript=m;exports.getMcpAppsAdapterScript=h;exports.postUIActionResult=R;exports.uiActionResultIntent=D;exports.uiActionResultLink=U;exports.uiActionResultNotification=E;exports.uiActionResultPrompt=C;exports.uiActionResultToolCall=b;exports.wrapHtmlWithAdapters=c;
