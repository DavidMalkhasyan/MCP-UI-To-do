const M = "ui/resourceUri", m = "mcpui.dev/ui-", h = `var __defProp = Object.defineProperty;
...
const I = `var __defProp = Object.defineProperty;
var __defNormalProp = (obj, key, value) => key in obj ? __defProp(obj, key, { enumerable: true, configurable: true, writable: true, value }) : obj[key] = value;
var __publicField = (obj, key, value) => {
  __defNormalProp(obj, typeof key !== "symbol" ? key + "" : key, value);
  return value;
...
function g(e) {
  ...
  if (e.uiMetadata || e.metadata) {
    const s = Object.fromEntries(
      Object.entries(e.uiMetadata ?? {}).map(([a, i]) => [
        `${m}${a}`,
        i
      ])
    );
...
function l(e) {
  ...
  if (typeof TextEncoder < "u" && typeof btoa < "u") {
    ...
    for (let t = 0; t < s.length; t += i)
      a += String.fromCharCode(...s.slice(t, t + i));
    return btoa(a);
  } else {
    console.warn(
...
function u(e, n) {
  ...
  if (n.appsSdk?.enabled) {
    const t = f(n.appsSdk.config);
    s.push(t);
  }
  if (n.mcpApps?.enabled) {
    const t = w(n.mcpApps.config);
    s.push(t);
  }
  if (s.length === 0)
    return e;
...
async function y(e, n) {
  let s;
  try {
    if (s = new URL(e), s.protocol !== "http:" && s.protocol !== "https:")
      throw new Error(`MCP-UI SDK: URL must use http or https protocol. Got: ${s.protocol}`);
  } catch (r) {
    throw new Error(
      `MCP-UI SDK: Invalid URL provided for externalUrl fetch: ${r instanceof Error ? r.message : String(r)}`
    );
  }
  let a;
  try {
    ...
    throw new Error(
      `MCP-UI SDK: Failed to fetch external URL: ${r instanceof Error ? r.message : String(r)}`
    );
  }
  const i = a.headers.get("content-type") || "";
  !i.includes("text/html") && !i.includes("application/xhtml") && console.warn(
    `MCP-UI SDK: External URL content-type is not HTML: ${i}. Proceeding anyway.`
  );
  let t;
  try {
    t = await a.text();
...
<body>
  ...
  if (!a.ok)
    console.warn(
      `MCP-UI SDK: External URL returned non-OK status: ${a.status} ${a.statusText}. Proceeding with response body.`
    ), (!t || t.trim() === "") && (t = `<!DOCTYPE html>
<html>
<head>
  <title>Error ${a.status}</title>
</head>
<body>
  ...
  t = t.replace(/<base[^>]*>/gi, ""), t = t.replace(
    /<meta[^>]*http-equiv\s*=\s*["']?content-security-policy["']?[^>]*>/gi,
    "<!-- CSP meta tag removed by MCP-UI -->"
  );
  const o = s.pathname.substring(0, s.pathname.lastIndexOf("/") + 1) || "/", d = `<base href="${`${s.origin}${o}`}">`;
  return t.includes("<head>") ? t = t.replace("<head>", `<head>
...
function A(e) {
  ...
  if (e.content.type === "rawHtml") {
    ...
    e.adapters ? (n = u(n, e.adapters), s = c(e.adapters) ?? "text/html") : s = "text/html";
    ...
    if (e.adapters && (e.adapters.appsSdk?.enabled || e.adapters.mcpApps?.enabled))
      return (async () => {
        const t = await y(i, e.adapters);
        let o;
        e.adapters ? o = c(e.adapters) ?? "text/html" : o = "text/html";
        let p;
        switch (e.encoding) {
          case "text":
            p = {
          ...
          default: {
            const d = e.encoding;
            throw new Error(`MCP-UI SDK: Invalid encoding type: ${d}`);
          }
        }
        return {
          type: "resource",
...
export {
  P as InternalMessageType,
  M as RESOURCE_URI_META_KEY,
  S as ReservedUrlParams,
  A as createUIResource,
  c as getAdapterMimeType,
  f as getAppsSdkAdapterScript,
  w as getMcpAppsAdapterScript,
  R as postUIActionResult,
  U as uiActionResultIntent,
  D as uiActionResultLink,
  E as uiActionResultNotification,
  C as uiActionResultPrompt,
  b as uiActionResultToolCall,
  u as wrapHtmlWithAdapters
};
